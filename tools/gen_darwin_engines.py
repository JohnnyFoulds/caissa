...
NATIVE_KEYS = {"stockfish", "lc0", "irina", "drawfish"} | {
...
def write_wrapper(engine_dir: Path, exe_name: str, mode: str, abs_exe: Path):
    """Write a self-locating shell wrapper that calls lc-engine.

    The wrapper resolves the repo root at runtime so it works on any machine
    regardless of where the repo was cloned.  The engine path is expressed
    relative to the repo root so it is also relocatable.
    """
    wrapper = engine_dir / exe_name
    wrapper.parent.mkdir(parents=True, exist_ok=True)

    # wrapper is at bin/OS/darwin/Engines/<folder>/<exe> — five levels down from repo root
    try:
        rel_exe = abs_exe.relative_to(REPO)
        engine_ref = '"$REPO/' + str(rel_exe).replace("\\", "/") + '"'
    except ValueError:
        # abs_exe is outside the repo (unusual) — fall back to baked-in absolute path
        engine_ref = f'"{abs_exe}"'

    wrapper.write_text(textwrap.dedent(f"""\
        #!/bin/sh
        # Auto-generated by gen_darwin_engines.py — mode={mode}
        REPO="$(cd "$(dirname "$0")/../../../../.." && pwd)"
        exec "$REPO/tools/lc-engine" {mode} {engine_ref} "$@"
    """))
    wrapper.chmod(0o755)
    return wrapper
