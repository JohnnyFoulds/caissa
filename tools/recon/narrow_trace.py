"""Narrow trace: log every instruction that changes A4, with full context.

Runs with CMP-only hooks. The global trace hook logs:
  - Every instruction that modifies A4 (first occurrence)
  - The 20 instructions before the first A4 change
  - Disassembly of each captured instruction
"""
...
def _hook_cmpiw(_emu, addr, _sz, _u=None):
    ...
    if op != 'cmp': return  # CMP-only for this test
    try:
...
print('CMP-only hooks: %d' % n_cmp)

# ---- Narrow trace ----
md = capstone.Cs(capstone.CS_ARCH_M68K, capstone.CS_MODE_BIG_ENDIAN + capstone.CS_MODE_M68K_000)
REGS = [
    (M68K.UC_M68K_REG_A4, 'A4'), (M68K.UC_M68K_REG_A3, 'A3'),
    (M68K.UC_M68K_REG_A2, 'A2'), (M68K.UC_M68K_REG_A1, 'A1'),
    (M68K.UC_M68K_REG_A0, 'A0'), (M68K.UC_M68K_REG_D0, 'D0'),
    (M68K.UC_M68K_REG_D1, 'D1'), (M68K.UC_M68K_REG_D2, 'D2'),
    (M68K.UC_M68K_REG_D3, 'D3'), (M68K.UC_M68K_REG_D4, 'D4'),
]
_prev = {r: cpu._uc.reg_read(r) for r, _ in REGS}
_prev[M68K.UC_M68K_REG_A4] = A4  # force known starting value

_history = deque(maxlen=30)  # ring buffer of last 30 instructions
_done = [False]

def _read_regs(emu):
    return {r: emu.reg_read(r) for r, _ in REGS}

def _dis(emu, addr):
    try:
        raw = bytes(emu.mem_read(addr, 8))
        ins = list(md.disasm(raw, addr))
        return (ins[0].mnemonic + ' ' + ins[0].op_str) if ins else '???'
    except:
        return '???'

def _fmt_regs(rv):
    return ' '.join('%s=%08X' % (n, rv[r]) for r, n in REGS)

def trace_hook(emu, addr, sz, _u):
    if _done[0]:
        return
    regs = _read_regs(emu)
    entry = (addr, regs)
    ...
    prev_a4 = _prev.get(M68K.UC_M68K_REG_A4, A4)
    cur_a4 = regs[M68K.UC_M68K_REG_A4]
    if cur_a4 != prev_a4:
        _done[0] = True
        print('\n=== A4 CHANGED: 0x%X -> 0x%X at PC=0x%05X ===' % (prev_a4, cur_a4, addr))
        print('Last %d instructions before change:' % len(_history))
        for h_addr, h_regs in _history:
            dis = _dis(emu, h_addr)
            print('  PC=0x%05X  %-35s  %s' % (h_addr, dis, _fmt_regs(h_regs)))
        print('\nCurrent state at PC=0x%05X: %s' % (addr, _fmt_regs(regs)))
        # Also show disassembly of current instruction's raw bytes
        try:
            raw = bytes(emu.mem_read(addr, 8))
            print('Raw bytes at PC: %s' % raw.hex())
        except: pass
        emu.emu_stop()
...
uc = cpu._uc
uc.hook_add(unicorn.UC_HOOK_CODE, trace_hook)

print('Running narrow trace (stops at first A4 change)...')
try:
    cpu.emu_start(AI_OUTER_DRIVER_ADDR, until=SENTINEL, count=500_000_000)
    pc_ = uc.reg_read(M68K.UC_M68K_REG_PC)
    if not _done[0]:
        print('Emulation stopped at PC=0x%X without A4 change!' % pc_)
