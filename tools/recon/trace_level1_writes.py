#!/usr/bin/env python3
"""Trace all key writes with level=1 (zero [04AD5]) to find the [012B6] bottleneck."""
...
def write_hook_low(emu, access, address, size, value, _):
    pc = emu.reg_read(m68k.UC_M68K_REG_PC)
    rec = f"  step={STEP[0]:6d} [0x{address:05X}]={value:#04x}  PC=0x{pc:05X}: {dis1(pc)}"
    if address == 0x025C:
        WRITES_025C.append(rec)
    elif address == 0x012B6 or address == 0x012B7:
        WRITES_012B6.append(rec)
    elif address == 0x025F:
        WRITES_025F.append(rec)
        if value == 1:
            mf = bytes(emu.mem_read(0x04AD2, 1))[0]
            mt = bytes(emu.mem_read(0x04AD3, 1))[0]
            print(f"\n  *** MOVE at step {STEP[0]}: PC=0x{pc:05X} from=0x{mf:02X} to=0x{mt:02X} ***")
            emu.emu_stop()

cpu._uc.hook_add(UC_HOOK_MEM_WRITE, write_hook_low, begin=0x0000, end=0x1300)
cpu._uc.hook_add(UC_HOOK_MEM_WRITE, write_hook_low, begin=0x04AD0, end=0x04AE0)
...
def code_hook(emu, addr, size, _):
    ...
    if STEP[0] % 200_000 == 0:
        ...
        STEP_LOG.append((STEP[0], addr, v025C, v012B6))
cpu._uc.hook_add(UC_HOOK_CODE, code_hook)

t0 = time.perf_counter()
print("Running 3M steps with level=1, tracing writes to [025C], [012B6], [025F]...", flush=True)
try:
    cpu.emu_start(AI_OUTER_DRIVER_ADDR, until=SENTINEL, count=3_000_000)
except Exception as e:
    print(f"Exception: {e}")
elapsed = time.perf_counter() - t0

print(f"\nDone in {elapsed:.2f}s  steps={STEP[0]}")
print(f"\n=== Writes to [025C] ({len(WRITES_025C)} total, first 20) ===")
for w in WRITES_025C[:20]: print(w)
print(f"\n=== Writes to [012B6] ({len(WRITES_012B6)} total) ===")
for w in WRITES_012B6[:20]: print(w)
print(f"\n=== Writes to [025F] ({len(WRITES_025F)} total) ===")
for w in WRITES_025F[:20]: print(w)
print(f"\n=== Step log (every 200K steps) ===")
for step, pc, c, b6 in STEP_LOG:
    print(f"  step={step:7d}  PC=0x{pc:05X}  [025C]={c:#x}  [012B6]={b6:#x}")
